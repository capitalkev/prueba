...
class OperationService:
    ...
    async def process_operation_sync(self, operation_data: dict, db: Session):
        ...
        try:
            ...
            # 3. Generar operation_id temprano para Drive
            from repository import OperationRepository
            repo = OperationRepository(db)
            operation_id = repo.generar_siguiente_id_operacion()
            
            # 3. Publicar a Drive en paralelo con operation_id
            drive_payload = {**operation_data, "operation_id": operation_id}
            ...
            await self.wait_for_drive_and_finalize(tracking_id, operation_data, parsed_results, cavali_results, db, operation_id)
            
        except Exception as e:
            logging.error(f"SYNC: Error procesando {operation_data.get('tracking_id')}: {e}")
            raise
    
    async def wait_for_drive_and_finalize(self, tracking_id: str, operation_data: dict, parsed_results: dict, cavali_results: dict, db: Session, operation_id: str):
        """Espera el resultado de Drive y finaliza la operación"""
        try:
            # Insertar datos iniciales en staging con operation_id
    ...
    def process_final_operation(self, payload: dict, db: Session):
        ...
        try:
            ...
            # Usar el operation_id ya generado desde staging
            operation_id = staging_record.operation_id or repo.generar_siguiente_id_operacion()
