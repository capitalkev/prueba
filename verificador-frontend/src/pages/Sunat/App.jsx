...
export default function App() {
  ...
  const { clients, errorData: clientsError } = useClients(
    firebaseUser,
    selectedUserEmails,
    users
  );
  ...
  useEffect(() => {
    if (clients.length > 0 && selectedClientIds.length > 0) {
      ...
      const validSelectedIds = selectedClientIds.filter((id) =>
        availableClientIds.has(id)
      );
  ...
  const invoices = useMemo(() => {
    const transformed = ventas.map((venta) => {
      const invoiceId = `${venta.serie_cdp || ""}-${
        venta.nro_cp_inicial || venta.id
      }`;
      ...
      const notaCreditoMonto = venta.nota_credito_monto
        ? parseFloat(venta.nota_credito_monto)
        : null;
      const montoNeto =
        venta.monto_neto !== undefined && venta.monto_neto !== null
          ? parseFloat(venta.monto_neto)
          : amount;
  ...
  const monthlyMetrics = useMemo(() => {
    console.log(
      "📈 [App.jsx] Calculando monthlyMetrics desde metrics:",
    ...
    console.log(
      "📈 [App.jsx] monthlyMetrics calculado:",
    ...
    try {
      ...
      if (newStatus === "Perdida" && lossReason) {
        const response = await fetch(
          `${API_BASE_URL}/api/ventas/${ventaId}/perdida`,
          {
            method: "PUT",
            headers: {
        ...
        const response = await fetch(
          `${API_BASE_URL}/api/ventas/${ventaId}/estado`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
      ...
      // Forzar recarga de datos para actualizar métricas y tabla
    ...
    return (
      <div className="bg-gray-100 font-sans h-screen w-full flex items-center justify-center">
        <div className="text-center bg-white p-8 rounded-lg shadow-md">
          ...
          <button
            onClick={async () => await signOut(auth)}
            className="mt-2 ml-2 text-sm text-gray-600 hover:underline"
          >
            Cerrar Sesión
          </button>
        </div>
